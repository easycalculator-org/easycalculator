 (function() {
      ...
      function computeStats(geojson) {
        ...
        function calculateArea(ring) {
          ...
          const areaM2 = area * (111320 * 111320) * Math.cos(avgLat);
          return areaM2;
        }
      ...
      function processKMLText(kmlText, fileName) {
        try {
          ...
          console.log('✅ GeoJSON from KMZ:', geojson);
          ...
          if (featureCount === 0) {
            showToast('KMZ contains no features.', 'error');
            return;
          }
          ...
          showToast(`KMZ loaded: ${featureCount} feature(s)`, 'success');
        } catch (err) {
          console.error('❌ KMZ processing error:', err);
          showToast('Error processing KMZ: ' + err.message, 'error');
          resetStats();
          ...
          try {
            ...
            JSZip.loadAsync(arrayBuffer).then(function(zip) {
              ...
              kmlFile.async('string').then(function(kmlText) {
                processKMLText(kmlText, kmlFileName);
              }).catch(function(err) {
                throw new Error('Failed to extract KML: ' + err.message);
              });
            }).catch(function(err) {
              throw new Error('Failed to unzip KMZ: ' + err.message);
            });
          } catch (err) {
      ...
      async function downloadPng() {
        if (!isLoaded || !geoJsonLayer) {
          showToast('Please load a KMZ file first.', 'error');
          return;
        }
      ...
      function handleFile(file) {
        ...
        if (ext !== 'kmz') {
          showToast('Please select a .kmz file.', 'error');
          return;
        }
      ...
      btnFitBounds.addEventListener('click', function() {
        if (geoJsonLayer && isLoaded) {
          ...
          showToast('Please load a KMZ first.', 'error');
        }
      });
      ...
      console.log('🗺️ KMZ Viewer ready — upload a .kmz file.');
