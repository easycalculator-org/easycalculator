...
async function getCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
        try {
            ...
            infoDiv.innerHTML = `
                <div class="h4 text-dark">${formatElevation(elevationNum)}</div>
                <div class="small">
            ...
            currentLocationMarker = L.marker([lat, lng], {
                icon: L.divIcon({
                    className: 'elevation-marker',
                    html: `<div class="bg-primary fw-semibold fs-3">${formatElevation(elevationNum)}</div>`
                })
            }).addTo(map);
...
map.on('click', async (e) => {
    ...
    try {
        ...
        const marker = L.marker([lat, lng], {
            icon: L.divIcon({
                className: 'elevation-marker',
                html: `<div class="bg-secondary text-dark fs-4">${formatElevation(elevationNum)}</div>`
            })
        }).addTo(map);
...
function addRecentLocation(location) {
    ...
    const elevationNum = parseFloat(location.elevation);
    entry.innerHTML = `
        <div>
            <strong class="${location.type === 'current' ? 'text-primary' : 'text-secondary'}">${formatElevation(elevationNum)}</strong>
